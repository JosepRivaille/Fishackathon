...
const filterLaws = (today, attrs) => {
    /*const todayDate = new Date(today);
    return annoyAsync.filter(({timeWindow, affects, affectsIn}) => {
        const condAF = !affects.length || affects.some(a1 => attrs.affects.some(a2 => a1 === a2));
        const condTW = timeWindow === null || (todayDate >= timeWindow.start && todayDate <= timeWindow.end);
        const condAI = affectsIn === 0 || todayDate - attrs.start;
        console.log(123);
        return true;
        return condAF && condTW && condAI;
    });*/
    console.log(annoyAsync.length);
    return annoyAsync;
};

export default filterLaws;